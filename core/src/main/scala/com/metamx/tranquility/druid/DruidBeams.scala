...
import com.metamx.common.scala.untyped.dict
import com.metamx.common.scala.untyped.normalize
...
object DruidBeams
{
  ...
  {
    builderFromSpecScala(
      environment,
      dict(
        normalize(
          DruidGuicer.objectMapper.convertValue(
            fireDepartment,
  ...
  /**
    ...
    * @return a new builder
    */
  def builderFromSpecScala(
    environment: DruidEnvironment,
    specMap: Map[String, Any]
  ): Builder[Map[String, Any]] =
  {
    builderFromSpec(
      (d: Map[String, Any]) => d.asJava.asInstanceOf[java.util.Map[String, AnyRef]],
      environment,
      specMap
    )
  }
  ...
  /**
    * Start a builder for Java Maps based on a particular Druid realtime spec. Not all of the realtime spec
    * is used, but we do translate as much as possible into DruidBeams configurations. The builder generated by this
    * method will already have a [[Builder.tuning]], [[Builder.druidTuning]], [[Builder.location]], [[Builder.rollup]],
    * and [[Builder.timestampSpec]] configured.
    *
    ...
    * @return a new builder
    */
  def builderFromSpecJava(
    environment: DruidEnvironment,
    specMap: java.util.Map[String, AnyRef]
  ): Builder[java.util.Map[String, AnyRef]] =
  {
    builderFromSpec(
      identity,
      environment,
      dict(normalize(specMap))
    )
  }
  ...
  ): Builder[MessageType] =
  {
    ...
    val timeFn = (d: MessageType) => {
      Option(timestampSpec.extractTimestamp(messageToJavaMap(d))).getOrElse(epoch)
    }
    ...
    builder(timeFn)
      ...
      .druidTuning(
        Option(specMap.getOrElse("tuningConfig", null)) map { tuningMap =>
          DefaultScalaObjectMapper.convertValue(tuningMap, classOf[DruidTuning])
        } getOrElse {
          DruidTuning()
        }
      )
